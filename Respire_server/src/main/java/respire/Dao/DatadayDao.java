...
public interface DatadayDao extends PagingAndSortingRepository<Datanow,DatanowPK> {
	...
	@Query("select new respire.Result.DataModle(time,pm25,so2,co2,1) from Dataday where userid=?1 and date_format(now(), '%Y-%m-%d')>date_format(time, '%Y-%m-%d')  ORDER BY time desc")
	public List<DataModle> getbyweek1(long userid, Pageable pageable);
	default List<DataModle> getbyweek(long userid){
		return getbyweek1(userid, new PageRequest(0,7));
	}
	
	@Query("select new respire.Result.DataModle(time,pm25,so2,co2,2) from Dataday where userid=?1 and date_format(now(), '%Y-%m')=date_format(time, '%Y-%m') ORDER BY time desc")
	public List<DataModle> getbymonth(long userid);
	
	@Query("select new respire.Result.DataModle(time,Avg(pm25) as pm25, Avg(so2) as so2,Avg(co2) as co2,3) from Dataday d where date_format(now(),'%Y')=DATE_FORMAT(time,'%Y') and d.userid=?1 group by date_format(time, '%Y-%m') ORDER BY time desc") 
